...
if [ $? -eq 0 ]; then
  jupyter --version
  echo "Already Exists Jupyter Notebook"
  exit 0
fi
...
echo "jupyter notebook --ip=* --no-browser --NotebookApp.token=''"
echo "jupyter notebook --ip=* --no-browser --NotebookApp.token='hogehoge'"
