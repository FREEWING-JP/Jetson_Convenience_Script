...
if [ $? = 0 ]; then
  ...
  if [[ ${TF_VERSION} =~ ^([0-9]+)\..*$ ]]; then
    ...
    if [ ! "${BASH_REMATCH[1]}" = "1" ]; then
      echo "Already Installed TensorFlow v1.x"
      exit 0
    fi
  fi
...
if [ $? = 0 ]; then
  echo "JetPack 4.5"
  TF_URL="https://developer.download.nvidia.com/compute/redist/jp/v45"
fi
...
chmod +x sub_TensorFlow_v1.sh
bash ./sub_TensorFlow_v1.sh $TF_URL
